...
function scr_ability_pressed(type){
	...
	if(using) {
	if(variable_global_get(type) == "Hook") {
		held = true
		range = 0;
	}
	if(variable_global_get(type) == "Curse" && ball_player.curseCharge > 0) 
		global.stun = 0;
	...
	if(global.ammo > 0 && instance_exists(ball_player) && variable_global_get(type + "Cool") == 0 && global.stun == 0 && global.shop == false && global.dead == false) {
		var cool = 1;
	    switch(variable_global_get(type)) {
	        ...
	        case "heal":
	            if(global.ammo > 2) {
					scr_ball_ammo(3)
					node_send(buffer,"eventName","Bullet","Num",ball_player.num,"X", ball_player.x, "Y", ball_player.y, "Obj", obj_healthShot, "Dir", point_direction(ball_player.x,ball_player.y,mouse_x,mouse_y))				
					cool = 4;
				}
	        break;
	        case "Blood Shot":
					scr_ball_ammo(1)
					node_send(buffer,"eventName","Bullet","Num",ball_player.num,"X", ball_player.x, "Y", ball_player.y, "Obj", obj_bloodShot, "Dir", point_direction(ball_player.x,ball_player.y,mouse_x,mouse_y))	
					cool = 4;
	        ...
	        case "Rocket":
					...
					scr_ball_ammo(2)
					node_send(buffer,"eventName","Bullet","Num",ball_player.num,"X",xp, "Y", yp, "Obj", obj_rocket, "Dir", dir)						cool = 14;
	        break;
	        case "Bump":
				scr_ball_ammo(1)
				node_send(buffer,"eventName","Bullet","Num",ball_player.num,"X", ball_player.x, "Y", ball_player.y, "Obj", obj_bump, "Dir", point_direction(ball_player.x,ball_player.y,mouse_x,mouse_y))	
				cool = 8;
	        break;
	        case "Armor Break":
	            if(global.ammo > 1) {
					scr_ball_ammo(2)
					node_send(buffer,"eventName","Bullet","Num",ball_player.num,"X", ball_player.x, "Y", ball_player.y, "Obj", obj_spear, "Dir", point_direction(ball_player.x,ball_player.y,mouse_x,mouse_y))	
					cool = 13;
				}
	        break;
	        case "Frost Shot":
			...
			case "Poison Dagger":
				if(global.ammo > 1) {
					node_send(buffer,"eventName","Bullet","Num",ball_player.num,"X", ball_player.x, "Y", ball_player.y, "Obj", obj_knife, "Dir", point_direction(ball_player.x,ball_player.y,mouse_x,mouse_y))
		            scr_ball_ammo(2);
					cool = 8;
				}
			break;
