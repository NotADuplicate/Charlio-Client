	...
	if(global.invincibility == 0 && global.dead == false) {
		...
		if(global.sponge != -1 && point_distance(x,y,global.sponge.x,global.sponge.y) < 300) { //deal damage to tank instead
			/*with(ball_game) {
			    ...
			    buffer_write(buffer, buffer_u8, global.sponge.num);
		...
		/*if(ball_player.milked > 0 && argument[1] != ball_player.num && argument[1] < 10) { //give health for milk
					with(ball_game) {
					    buffer_seek(buffer, buffer_seek_start, 0);
					    buffer_write(buffer, buffer_u8, 28);
					    buffer_write(buffer, buffer_u8, argument[1]);
						buffer_write(buffer,buffer_u8,round(dmg*global.players[ball_player.milkNum].healing));
					    buffer_write(buffer, buffer_u8, 2);
					    network_send_packet(socket, buffer, buffer_tell(buffer));
					}
		}
		if(argument[1] != ball_player.num && argument[1] < 10 && global.players[argument[1]].lifeSteal > 0) {
					with(ball_game) {
					    buffer_seek(buffer, buffer_seek_start, 0);
					    buffer_write(buffer, buffer_u8, 28);
					    buffer_write(buffer, buffer_u8, argument[1]);
						buffer_write(buffer,buffer_u8,round(dmg*global.players[argument[1]].lifeSteal))
					    buffer_write(buffer, buffer_u8, 2);
					    network_send_packet(socket, buffer, buffer_tell(buffer));
					}
		}*/
