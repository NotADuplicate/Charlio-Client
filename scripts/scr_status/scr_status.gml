...
function scr_status(status,target){
	switch(status) {
		...
		case 3: //bleed
			if(global.players[target].spellShield == 0)
				global.players[target].bleed = 120;
		break;
		...
		case 16:
			if(global.players[target].spellShield == 0) {
				if(target == ball_player.num && ball_player.broken = 0) {
		...
		case 17: //"
			...
			if(global.players[target].spellShield == 0) {
				...
				global.players[target].frost = 180;
		...
		break;
		case 32: //milknum
			global.players[target].milked = 300;
		break;
		case 33:
		...
		break;
		case 36:
			global.players[target].maxhp = round(global.players[target].maxhp*1.1)
			if(global.players[target].maxhp > 400)
				global.players[target].maxhp = 400
			global.players[target].image_xscale += .2;
			global.players[target].image_yscale += .175;
			global.players[target].scale += .1;
		break;
		case 37:
		...
		case 49: //refund shuriken
			if(target == ball_player.num) {
				...
				if(global.right == "Shuriken")
					global.rightCool = 0.5;
				if(global.space == "Shuriken")
					global.spaceCool = 0.5;
				if(global.Q == "Shuriken")
					global.QCool = 0.5;
				if(global.R == "Shuriken")
					global.RCool = 0.5;
			}
		break;
		case 50: //dashing
